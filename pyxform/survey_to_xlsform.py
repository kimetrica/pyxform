...
class XlsFormExporter():
    ...
    def record_question_data(self, question):
        ...
        if not isinstance(question, pyxform.question.MultipleChoiceQuestion):
            ...
            if xlsform_question_type not in \
                ...
                raise PyXFormError(
                    'Unexpected multiple-choice question type "{}"'.format(question['type']))

            # TODO: Would be nice to reuse the 'list name' when encountering reused sets of choices.
            # Generate a 'list name' comprised of the question name followed by 8
            # random bytes cast to string.
            ...
            survey_row[self.survey_sheet_columns.index(
                constants.TYPE)]= xlsform_question_type + ' ' + list_name
            ...
            if question.is_cascading_select():
                ...
                dict_to_insert= {constants.LIST_NAME: list_name,
                                 constants.NAME: self.CASCADING_SELECT_SAD_CHOICE_NAME}
                self.insert_dict_into_row(
                    dict_to_insert, cascading_select_sad_choices_row, self.choices_sheet_columns)
                ...
                if constants.LABEL in self.choices_sheet_columns:
                    cascading_select_sad_choices_row[
                        self.choices_sheet_columns.index(constants.LABEL)]= self.CASCADING_SELECT_SAD_CHOICE_LABEL
                else:
        ...
        if xlsform_question_type == constants.CALCULATE_XFORM:
            if CALCULATION_EXPORT_WARNING not in self.warnings:
                self.warnings.append(SKIP_LOGIC_EXPORT_WARNING)
            dict_to_insert= {constants.CALCULATE_XLSFORM:
                             question[constants.BIND][constants.CALCULATE_XFORM]}
        ...
        if (constants.BIND in question) and (constants.REQUIRED_XFORM in question[constants.BIND]):
            dict_to_insert= {
                constants.REQUIRED_XFORM: question[constants.BIND][constants.REQUIRED_XFORM]}
            self.insert_dict_into_row(dict_to_insert, survey_row, self.survey_sheet_columns)
    ...
    def record_question_choice(self, question_choice, list_name):
        ...
        # Record the entry for the mandatory 'name' column.
        choices_row[self.choices_sheet_columns.index(
            constants.NAME)]= question_choice[constants.NAME]

        # Record entries for the mandatory 'label' and/or 'label::X' columns.
        choice_label_dict= self.get_survey_element_label(question_choice)
        if not choice_label_dict:
            raise PyXFormError(
                'Choices for multiple-choice questions must have at least one label.')
...
# TODO: Move to `utilities` sub-package or some such.
def sheet_dict_to_xls(sheet_dict, path=None, warnings=None):
    '''
    Convert the provided sheet dictionary to an XLS-encoded XLSForm. Does the actual work of
    :py:func:`to_xls()`.

    :param dict sheet_dict:
...
def to_xls(survey, path=None, warnings=None):
    ...
    return sheet_dict_to_xls(sheet_dict, path, warnings)
...
def to_csv(survey, path=None, warnings=None, koboform=False):
    ...
    if koboform and ('label' not in survey_columns):
        ...
        if (warnings != None) and (len(label_columns) > 1):
            ...
            language_default_warning= 'Multiple translations are not supported in KoBoForm. Defaulting to language "{}".'.format(
                chosen_language)
    ...
    for sheet_name, sheet_rows in sheet_dict.iteritems():
        ...
        for row in sheet_rows:
            # Write out each row of data prepended with an empty cell and ensuring all
            # cells are UTF-8 encoded.
